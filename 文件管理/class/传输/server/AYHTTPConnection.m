...
- (void) processStartOfPartWithHeader:(MultipartMessageHeader *)header
{
    ...
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString * inBoxPath = [[paths objectAtIndex:0] stringByAppendingPathComponent:@"接收"];
  BOOL flog =  [FileManageCommon createFolderIfNotExisting:inBoxPath];
    if (flog) {
        NSLog(@"已经创建好或者已经存在");
    }else{
        NSLog(@"创建s失败");
    }
    NSString *uploadFilePath = [inBoxPath stringByAppendingPathComponent:fileName];
