...
{
    CGSize imgSize = self.size;
    CGFloat multiple_h =imgSize.height/newSize.height;
    CGFloat multiple_w =imgSize.width/newSize.width;
    CGFloat image_x =0.0;
    CGFloat image_y =0.0;
     UIGraphicsBeginImageContextWithOptions(newSize, NO, 0.0);
    if (multiple_h >  multiple_w) {
        
        image_x = (newSize.width -imgSize.width/multiple_h)/2;
         [self drawInRect:CGRectMake(image_x, 0, newSize.width, newSize.height)];
    }else{
        image_y = (newSize.height -imgSize.height/multiple_w)/2;
        [self drawInRect:CGRectMake(0, image_y, newSize.width, newSize.height)];
    }
...
{
    return [UIImage imageNamed:@"pub_default_circleAvatar.jpg"];
}
